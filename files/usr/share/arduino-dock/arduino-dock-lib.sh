...
_FlashApplication () {
	...
	# set the fuses
	avrdude -C $DOCK_LIB/$AVRDUDE_CONFIG -p atmega32u4 -c linuxgpio -P gpio -b 115200 -e -u -U lock:w:$UNLOCK:m -U efuse:w:$EFUSE:m -U hfuse:w:$HFUSE:m -U lfuse:w:$LFUSE:m
	AVRDUDE_RESULT=$?

	# flash the bootloader and lock the section
	if [ $AVRDUDE_RESULT -eq 0 ]
	then
		avrdude -C $DOCK_LIB/$AVRDUDE_CONFIG -p atmega32u4 -c linuxgpio -P gpio -b 115200 -U flash:w:$1 -U lock:w:$LOCK:m
		AVRDUDE_RESULT=$?
	fi
